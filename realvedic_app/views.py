...
def signUp(request,format=None):
    if request.method == 'POST':
        ...
        passw=make_password("1234")
        print(passw)
        if password != "null":
            enc_pass = make_password(password)
            token = make_password(email+last_name)
        else:
            enc_pass=password
            token = make_password(email+last_name)
...
def otp_login(contact):
    #otp twilio crediantials
    account_sid = "AC2abca168ea82d95684839f4ae777404a"
    auth_token = "7f5a72bcff9f699f722e4b3fd0769f2e"
    verify_sid = "VA4eb2990028d4f05b08154d09f7dae2d3"
    verified_number = "+918360345844"
...
def login(request):
    ...
    if phone_code !="null" and phone_no!="null":
        contact=phone_code+phone_number
        otp_login(contact)
    try:
        user = user_data.objects.get(email = email)
        print("thi is pass",user.password)
    ...
    if client_id == "null":
        ...
        if client_id == user.google_client_id:
    ...
    if no_login_token != 'null':
        ...
        for i in no_login_cart:
            ...
            if len(user_cart_row) > 0 :
                ...
                no_login_cart.filter(product_id = i['product_id'],size = i['size']).update(user_id = user.id)# doubt
